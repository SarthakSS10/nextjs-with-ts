...
function UserRatings(props) {
  ...
  const albumVal = []
  const [trigger, { isLoading, isError, data, error } ] = fetchUserOnId.useLazyQuery();
  const [editRatings , {data:editResponse}] = useEditRatingsMutation()



  const ratingChanged = async(rating,albumId,id)=>{
  ...
  const selectedOptionChange = async(id)=>{
    console.log("val",id);
    setSelectedUser(id.value)
    let {data} = await trigger(id.value)
    ...
    setImages(()=>[...arr])
      ...
      const albumVal = images.map((val,index)=>{
        return {
          _id:val._id,
          url:val.url,
          rating:0
  ...
  useEffect(()=>{
    // dispatch(userFetch())
    const arrayObj = props.users.map(item => {
      return {
        value: item._id,
        label: item.name
      };
    ...
    const albumVal = props.album.map((val,index)=>{
      return {
        _id:val._id,
        url:val.url,
        rating:0
                ...
                <Card.Body>
                  <Card.Title>
                    <ReactStars
                      count={5}
                      value	={val.rating}
                      onChange={(id)=>ratingChanged(id,val._id,selectedUser)}
                      size={24}
                      activeColor="#ffd700"
                    />
                    <div>{val.rating}</div>
